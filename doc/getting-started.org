...
#+HTML: <div class="col-md-9"> 
 PongoDB is meant to be used with an existing relational database. In
 this guide the database that PongoDB was developed for will be used
 in examples.
#+HTML: </div> 
...
Note that it always returns JSON.

#+HEADER: :engine postgresql :cmdline "--port 5433 --host localhost --user maxclaims maxclaims"
#+BEGIN_SRC sql :results code
SELECT pongo.insert('
{
 "into" : "contract", 
 "values" : {"contract_number" : "TEST: This is a Contract Number"}, 
 "returning" : "contract.*"
}');
...
#+BEGIN_SRC json
  {
      "contract_id": 2324,
      "contract_number": "TEST: This is a Contract Number",
      "agency_id": null,
      "insurance_company_id": null,
      "effective_date": "2016-01-05",
      "expiry_date": "2017-01-05",
      "syndicate_id": null,
      "london_broker_id": null
  }
#+END_SRC

* Read :: Creation is for a purpose
...
#+BEGIN_SRC sql :results code
SELECT pongo.select('
["count(*)", "min(effective_date)", "max(expiry_date)",
 {"from" : "contract"}
]');
...
For the one just entered, the contract number is enough to select it.
...
[ "*", 
  {
   "from" : "contract", 
   "where" : {"contract_number" : "TEST: This is a Contract Number"}
  }
]');
...
*** Smooth Operators :: $like and $ilike

But, of course, that may not be all that is desired. A query is wanted
which searches for them, ordering by the top matches.

First, insertion of many test contracts.
...
SELECT pongo.insert('
{
 ...
 "values" : [
             ...
             {"contract_number" : "TEST: Another"}, 
             {"contract_number" : "TESTING 123"}, 
             {"contract_number" : "TEST ICICLE"}, 
             {"contract_number" : "TEST: Another 12345"}, 
             {"contract_number" : "TEST: C-k C-y"}, 
             {"contract_number" : "TEST IS DONE"}
            ],
...
#+BEGIN_SRC sql
SELECT pongo.select('
[ "''contract'' AS _type", "contract_id AS _id", 
  "contract.*",
  {
   "from" : "contract", 
   "where" : {"contract_number" : {"$like" : "TEST%"}},
   "order_by" : "contract_number = ''TEST'' DESC",
   "limit" : 2
  }
...
| select                                                                                                                                                                                                                                                   |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| {"_type":"contract","_id":2325,"contract_id":2325,"contract_number":"TEST","agency_id":null,"insurance_company_id":null,"effective_date":"2016-01-05","expiry_date":"2017-01-05","syndicate_id":null,"london_broker_id":null}                            |
| {"_type":"contract","_id":2324,"contract_id":2324,"contract_number":"TEST: This is a Contract Number","agency_id":null,"insurance_company_id":null,"effective_date":"2016-01-05","expiry_date":"2017-01-05","syndicate_id":null,"london_broker_id":null} |
** view :: A different perspective

 There is ~select({...})~, which returns a ~SETOF json~. While postgres
 does have many ways to access them, the entire JSON -> SQL -> JSON
 thing gets tiring. 

 [[file:view.org][~view~]] is a function which takes either an inclusive (only include
 these fields) or exclusive 'projection' (json object) as the second
 argument, and projects the object that way for you to view.

*** Inclusive 
 #+HEADER: :engine postgresql :cmdline "--port 5433 --host localhost --user maxclaims maxclaims" 
 #+BEGIN_SRC sql
   SELECT pongo.view(contract,    
    '{
 ...
 | view                                                                                                                           |
 |--------------------------------------------------------------------------------------------------------------------------------|
 | { "_id" : 2325, "_type" : "contract", "contract_number" : "TEST", "effective_date" : "2016-01-05" }                            |
 | { "_id" : 2324, "_type" : "contract", "contract_number" : "TEST: This is a Contract Number", "effective_date" : "2016-01-05" } |
 | { "_id" : 2323, "_type" : "contract", "contract_number" : "TEST: This is a Contract Number", "effective_date" : "2016-01-05" } |

 What is important to note is that even though ~_id~ and ~_type~ were
 not included, they still appear. Those fields are special to PongoDB,
 so even when they are not included in an inclusive projection, they
 still appear.
...
*** Exclusive 
 ...
 | view                                                                                                                                                                                            |
 |-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
 | { "_type" : "contract", "_id" : 2325, "contract_id" : 2325, "agency_id" : null, "insurance_company_id" : null, "expiry_date" : "2017-01-05", "syndicate_id" : null, "london_broker_id" : null } |
 | { "_type" : "contract", "_id" : 2324, "contract_id" : 2324, "agency_id" : null, "insurance_company_id" : null, "expiry_date" : "2017-01-05", "syndicate_id" : null, "london_broker_id" : null } |
 | { "_type" : "contract", "_id" : 2323, "contract_id" : 2323, "agency_id" : null, "insurance_company_id" : null, "expiry_date" : "2017-01-05", "syndicate_id" : null, "london_broker_id" : null } |

** as_json :: _id and _type, oh my!
<<as_json>>
 As previously mentioned, [[file:_id.org][~_id~]] and [[file:_type.org][~_type~]] are special to PongoDB. So
 much so that they have their own functions.

#+HEADER: :engine postgresql :cmdline "--port 5433 --host localhost --user maxclaims maxclaims" 
#+BEGIN_SRC sql
  SELECT pongo.select('
  [ "pongo._id(c.*)", "pongo._type(c.*)",
    {
     "from" : {"contract" : {"as" : "c"}},
     "where" : {"contract_number" : {"$like" : "TEST%"}},
     "limit" : 3
...
| contract                        |
|---------------------------------|
| {"_id":2325,"_type":"contract"} |
| {"_id":2324,"_type":"contract"} |
| {"_id":2323,"_type":"contract"} |

They are special enough that they are added, by default, using
[[file:as_json.org][~as_json(...)~]].

#+HEADER: :engine postgresql :cmdline "--port 5433 --host localhost --user maxclaims maxclaims" 
#+BEGIN_SRC sql
  SELECT pongo.select('
  [ "pongo.as_json(c.*) AS contract",
    {
     "from" : {"contract" : {"as" : "c"}},
     "where" : {"contract_number" : {"$like" : "TEST%"}},
     "limit" : 4, 
...
| contract                                                                                                                                                      |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| {"contract":{ "_type" : "contract", "_id" : 2326, "contract_number" : "TEST: Another", "effective_date" : "2016-01-05", "expiry_date" : "2017-01-05" }}       |
| {"contract":{ "_type" : "contract", "_id" : 2327, "contract_number" : "TESTING 123", "effective_date" : "2016-01-05", "expiry_date" : "2017-01-05" }}         |
| {"contract":{ "_type" : "contract", "_id" : 2328, "contract_number" : "TEST ICICLE", "effective_date" : "2016-01-05", "expiry_date" : "2017-01-05" }}         |
| {"contract":{ "_type" : "contract", "_id" : 2329, "contract_number" : "TEST: Another 12345", "effective_date" : "2016-01-05", "expiry_date" : "2017-01-05" }} | 

Another thing to notice is that nulls are stripped automatically, and
the primary key is removed as well as that is taken care of by ~_id~.

That can be configured in various ways.

#+HEADER: :engine postgresql :cmdline "--port 5433 --host localhost --user maxclaims maxclaims" 
#+BEGIN_SRC sql
  select pongo.default_as_json_options(null::contract); 
  ...
  WITH "options" AS (
   ...
   SELECT 'pongo.as_json(c.*, '''||(SELECT "options" FROM "options")||''') AS contract'
    AS "as_json"
  ), "select" AS (
    SELECT  '{
     "from" : {"contract" : {"as" : "c"}},
...
| select                                                      |
|-------------------------------------------------------------|
| {"contract":{ "_type" : "contract", "contract_id" : 2326 }} |
| {"contract":{ "_type" : "contract", "contract_id" : 2327 }} |
| {"contract":{ "_type" : "contract", "contract_id" : 2328 }} |
| {"contract":{ "_type" : "contract", "contract_id" : 2329 }} |

But again, that is a little to much PongoDB functions nested within
SQL syntax. 


** ~find(json)~ :: Top of the pops

Trying to use JSON as much as possible is the idea behind
PongoDB. While [[select][~select~]] takes a json syntax and returns a ~SETOF
json~, integrating [[as_json][~as_json(...)~]], ~json_agg~ and the like
causes SQL to rear its ugly head.

[[file:find.org][~find()~]] to the rescue!

There is a tonne of contracts. The requirement is to return a single
result, and not a ~SETOF~, as such a thing does not exist in JSON. We
could use the ~pongo.select(...)~ to return a JSON array.

#+HEADER: :engine postgresql :cmdline "--port 5433 --host localhost --user maxclaims maxclaims" 
#+BEGIN_SRC sql
  SELECT pongo."select"('
 ["json_agg(contracts) AS contracts", 
   {"from" : { 
       "$select": [ "pongo.as_json(c.*, ''{\"view\" : {\"_type\" : false, \"contract_number\" : true }}'') AS contracts",
    {
     "from" : {"contract" : {"as" : "c"}},
     "where" : {"contract_number" : {"$like" : "TEST%"}}, 
     "order_by" : {"contract_id" : {"$desc" : true}},
     "limit" : 2
    }

  ], "as" : "fa"}}]')->'contracts' AS contracts;
#+END_SRC

| contracts                                                                                                     |
|---------------------------------------------------------------------------------------------------------------|
| [{ "_id" : 2331, "contract_number" : "TEST IS DONE" }, { "_id" : 2330, "contract_number" : "TEST: C-k C-y" }] |


*** Find an array

This is done often enough that there is a function that defaults just to that.

We want contracts that match the string "TEST", in a certain order
#+HEADER: :engine postgresql :cmdline "--port 5433 --host localhost --user maxclaims maxclaims" 
#+BEGIN_SRC sql
  SELECT pongo."find"('
   { 
    "_type" : "contract",
    "view" : {"_type" : false, "contract_number" : true},
    "where" : {"$or" : {
                        "contract_number" : {"$ilike" : "%TEST%"}}},
    "order_by" : {
                  "contract_number" : {"$desc" : {"$eq" : "TEST"}}, 
                  "contract_number" : {"$desc" : {"$like" : "TEST%"}}, 
                  "contract_number" : {"$ilike" : "TEST%"}, 
...
| find                                                                                                                                                                                                                                                                          |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [{ "_id" : 2325, "contract_number" : "TEST" }, { "_id" : 2331, "contract_number" : "TEST IS DONE" }, { "_id" : 2330, "contract_number" : "TEST: C-k C-y" }, { "_id" : 2329, "contract_number" : "TEST: Another 12345" }, { "_id" : 2328, "contract_number" : "TEST ICICLE" }] |
...
Sometimes a single object is all that is desired. We could use ~->0~ ,
but there are multiple reasons why ~{"single" : true}~ is preferred.
...
| find                                                        |
|-------------------------------------------------------------|
| { "_id" : 2329, "contract_number" : "TEST: Another 12345" } |


*** Find a Page

At the very top is a [[file:find.org::Complex%20Object%20Paged%20Response%20Envelope][Paged Response Envelope]]. 
...
b#+HEADER: :engine postgresql :cmdline "--port 5433 --host localhost --user maxclaims maxclaims" 
#+BEGIN_SRC sql :results code
  SELECT pongo."find"('
   { 
    "_type" : "person",
...
#+BEGIN_SRC sql
find
[{ "_type" : "person", "_id" : 19, "first_name" : "System", "last_name" : "Administrator", "province_state_id" : 1, "date_modified" : "2000-04-08T00:00:00", "user_id" : "sysadmin", "user_password" : "jake" }]
#+END_SRC

* Update :: The more things change

The request is simple. "Can you transfer the timecards & attachments
from claim # 32586 to claim # 34184 please?".
...
SELECT '{
         "set" : {"claim_id" : 34184},
         "where" : {"claim_id" : 32586}
        }'::json 
   AS "update"
)

SELECT pongo.update(null::timecard, "update") AS timecard
       pongo.update(null::attachment, "update") AS attachment
FROM "request" ;
...
#+BEGIN_SRC sql
update	update
{"result_oid" : "0", "row_count" : 0}	{"result_oid" : "0", "row_count" : 24}
#+END_SRC
...
#+RESULTS:
#+begin_example
PSQL(1)                 PostgreSQL 9.4.5 Documentation                 PSQL(1)
   ...
   Meta-Commands
       ...
       \pset [ option [ value ] ]
           ...
           border
               ...
               and 2 (table frame) make sense.  latex and latex-longtable also
               support a border value of 3 which adds a dividing line between
               each row.
           ...
           format
               ...
               This might not be necessary in HTML, but in LaTeX you must have
               a complete document wrapper.  latex-longtable also requires the
               LaTeX longtable and booktabs packages.
